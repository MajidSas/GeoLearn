...
label_colors = np.array([cmap(float(i)/max_label)[:3] for i in range(max_label+1)])
_colors_tensor = torch.from_numpy(label_colors).to(torch.float32)
...
class SpatialDatasetSeg(Dataset):
    ...
    def get(self, idx):
        ...
        file = self.files[idx]
        parts = file.rsplit('/', 2)
        params = torch.tensor([float(num) for num in parts[-2].split('_')]).to(torch.float32)
...
def custom_collate_fn(batch):
    ...
    params = torch.stack([item[2] for item in batch], dim=0)
    points = torch.cat([item[3] for item in batch], dim=0)
...
class UNet(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=3):
        ...
        self.param_fc = nn.Sequential(
            nn.Linear(3, filters[1]),
            nn.ReLU(inplace=True),
            nn.Linear(filters[1], filters[2])  # Match to spatial feature dimensions
        )
    ...
    def forward(self, x, params):
        ...
        vector_features = self.param_fc(params)  # Output: (B, filters[2])
        vector_features = vector_features.unsqueeze(-1).unsqueeze(-1)  # Reshape to (B, filters[2], 1, 1)
        vector_features = vector_features.expand(-1, -1, e3.size(2), e3.size(3))  # Match spatial dims
        combined = e3 + vector_features
        d3 = self.Up3(combined)
        # print(params.shape, e3.shape, vector_features.shape, d3.shape)
...
model.load_state_dict(torch.load(unet_param_path, weights_only=True))
...
for batch_idx in progress_bar:
    input_histogram, output_histogram, params, points, actual_labels, batch = next(iter2)
    actual = actual_labels.cpu() if actual == None else torch.cat((actual, actual_labels.cpu()))
    batches = batch.cpu() if batches == None else torch.cat((batches, batch.cpu()))
    _batch = batch - ii
    ii+=batch_size
    params0 = params[:, 0][_batch].cpu() if params0 == None else torch.cat((params0, params[:, 0][_batch].cpu()))
    params1 = params[:, 1][_batch].cpu() if params1 == None else torch.cat((params1, params[:, 1][_batch].cpu()))
    params2 = params[:, 2][_batch].cpu() if params2 == None else torch.cat((params2, params[:, 2][_batch].cpu()))
    input_histogram, output_histogram, params, actual_labels  = input_histogram.to(device), output_histogram.to(device), params.to(device), actual_labels.to(device)
    with torch.no_grad():
        torch.cuda.synchronize()
        start_event.record()
        prediction = model(input_histogram, params)
        end_event.record()
        torch.cuda.synchronize()
...
print('Clustering parameterized unet time: ', total_time)

df = pd.DataFrame()
df["batches"] = batches.cpu().numpy().flatten()
df["params_0"] = params0.cpu().numpy().flatten()
df["params_1"] = params1.cpu().numpy().flatten()
df["params_2"] = params2.cpu().numpy().flatten()
df["actual"] = actual.cpu().numpy().flatten()
df["predicted"] = predictions.cpu().numpy().flatten()

df.to_csv(results_folder + '/unet_param.csv', index=False)