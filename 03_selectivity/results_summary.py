...
p = pd.read_csv('/rhome/msaee007/bigdata/pointnet_data/selectivity_exp_output/pointnet_selectivity_0.577909.csv').clip(lower=0).clip(upper=1.0)
r = pd.read_csv('/rhome/msaee007/bigdata/pointnet_data/synthetic_data/main_exp_outputs/resnet_selectivity_1.218353.csv').clip(lower=0).clip(upper=1.0)
...
pd.DataFrame(combined_wmape).to_csv('/rhome/msaee007/PointNet/03_selectivity/results_summary_selectivity.csv', float_format="%.2e", index=False)
